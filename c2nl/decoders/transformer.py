...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.context_attn1 = MultiHeadedAttention(
            heads, d_model, d_k, d_v, dropout=dropout,
            coverage=coverage_attn)
    ...
    def forward(self,
        ...
        mid, attn, coverage = self.context_attn(memory_bank,  # MultiHeadedAttention
                                                memory_bank,
                                                query_norm,
                                                mask=src_pad_mask,
                                                layer_cache=layer_cache,
        ...
        mid, attn, coverage = self.context_attn1(memory_bank1,
                                                 memory_bank1,
                                                 mid_norm,
                                                 mask=src_pad_mask1,
                                                 ...
                                                 step=step,
                                                 coverage=coverage)
        mid_norm = self.layer_norm_1(self.drop(mid) + mid_norm)
...
class TransformerDecoder(DecoderBase):
    ...
    def init_state(self, src_len, max_len,src_len1,max_len1): #
        """Initialize decoder state."""
        state = dict()
        state["src_len"] = src_len  # [B]
        state["src_max_len"] = max_len  # an integer
        state["src_len1"] = src_len1  # [B]
        state["src_max_len1"] = max_len1  # an integer
        state["cache"] = None
        state["cache1"] = None
        return state
    ...
    def forward(self,
        ...
        src_pad_mask1 = ~sequence_mask(state["src_len1"],
                                      max_len=state["src_max_len1"]).unsqueeze(1)
        ...
        for i, layer in enumerate(self.layer):
            ...
            layer_cache1 = state["cache1"]["layer_{}".format(i)] \
                if step is not None else None
    ...
    def _init_cache(self, state):
        ...
        for i, layer in enumerate(self.layer):
            ...
            layer_cache["self_keys"] = None
            layer_cache["self_values"] = None
            state["cache1"]["layer_{}".format(i)] = layer_cache
