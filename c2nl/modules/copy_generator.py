...
class CopyGenerator(nn.Module):
    ...
    * :math:`p_{copy}` the probility of copying a particular word.
    ...
    def forward(self, hidden, attn, src_map):
        ...
        mul_attn = torch.mul(drop_attn, p_copy.expand_as(drop_attn))
...
class CopyGeneratorCriterion(object):
    ...
    def __call__(self, scores, align, target):
        ...
        # champed_scores = torch.clamp(scores, 0, )
        align_info = align.view(-1, 1) + self.offset
        champed_align_info = torch.clamp(align_info, 0, scores.size(1) - 1)
        #  out = scores.gather(1, align_info).view(-1)
        out = scores.gather(1, champed_align_info).view(-1)
