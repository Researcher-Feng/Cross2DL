...
class Code2NaturalLanguage(object):
    ...
    def update_model(self, ex):
        ...
        code_ex = {}
        for i_key, i_ex in ex['final_src'].items():
            code_ex[i_key] = {}
            # To enable copy attn, collect source map and alignment info
            if self.args.copy_attn:
                assert 'src_map' in i_ex

                source_map = make_src_map(i_ex['src_map'])
                source_map = source_map.cuda(non_blocking=True) if self.use_cuda \
                    else source_map

                blank, fill = collapse_copy_scores(self.tgt_dict, i_ex['src_vocab'])

            code_word_rep = i_ex['code_word_rep']
            code_char_rep = i_ex['code_char_rep']
            code_type_rep = i_ex['code_type_rep']
            code_mask_rep = i_ex['code_mask_rep']
            ...
            code_ex[i_key]['code_len'] = code_len
            code_ex[i_key]['code_word_rep'] = code_word_rep
            code_ex[i_key]['code_char_rep'] = code_char_rep
            code_ex[i_key]['code_type_rep'] = code_type_rep
            code_ex[i_key]['code_mask_rep'] = code_mask_rep
            code_ex[i_key]['source_map'] = source_map

        assert 'alignment' in ex
        ...
        # Run forward
        net_loss = self.network(code_ex=code_ex,
    ...
    def model_predict(self, ex, replace_unk=False):
        ...
        for i_key, i_ex in ex['final_src'].items():
            ...
            code_ex[i_key]['code_mask_rep'] = code_mask_rep
            code_ex[i_key]['source_map'] = source_map
            code_ex[i_key]['blank'] = blank
            code_ex[i_key]['fill'] = fill

        decoder_out = self.network(code_ex=code_ex,
                                   summ_word_rep=None,
                                   summ_char_rep=None,
                                   summ_len=None,
                                   tgt_seq=None,
        ...
        if replace_unk:
            for i in range(len(predictions)):
                ...
                predictions[i] = replace_unknown(predictions[i],
                                                 enc_dec_attn,
                                                 src_raw=ex['final_src']['rep_0']['code_tokens'][i])
                if self.args.uncase:
